...
public class GarageManager {
    ...
    public static GarageManager getInstance() { return INSTANCE; }


    private final List<Order> orders = new ArrayList<>();
    public MasterService masters = new MasterService();
    public WorkplaceService workplaces = new WorkplaceService();

    public int addOrder(Order order) {
        // Чек на место
        // Чек на то, что оно вообще существует
        Workplace workplace = workplaces.show().stream()
                .filter(item -> item.getId().equals(order.workplaceId))
                .findFirst()
                .orElse(null);
        if (workplace == null) {
        ...
        boolean fitsInWorkingHoursOfPlace = workplace.getWorkingSchedule().stream()
                .anyMatch(day -> !order.timeInterval.start().isBefore(day.start()) &&  // order.start >= day.start
                    !order.timeInterval.end().isAfter(day.end())  // order.end <= day.end
                );
        ...
        List<Order> ordersOnSelectedWorkplace = orders.stream()
                .filter(item -> item.workplaceId.equals(order.workplaceId))
                .toList();
        boolean hasConflict = ordersOnSelectedWorkplace.stream()
                .anyMatch(item -> item.status == Status.OPENED &&
                        item.timeInterval.start().isBefore(order.timeInterval.end()) &&
                        item.timeInterval.end().isAfter(order.timeInterval.start())
                );
        if (hasConflict) {
        ...
        for (Master master : currentMasters) {
            boolean fitsInWorkingHours = master.getWorkingSchedule().stream()
                    .anyMatch(day -> !order.timeInterval.start().isBefore(day.start()) &&  // order.start >= day.start
                    !order.timeInterval.end().isAfter(day.end())  // order.end <= day.end
            );
        ...
        for (Master master : currentMasters) {
            ...
            boolean conflict = ordersOfMaster.stream()
                    .anyMatch(existing -> existing.status == Status.OPENED &&
                            existing.timeInterval.end().isAfter(order.timeInterval.start()) &&
                            existing.getStartTime().isBefore(order.timeInterval.end())
                    );
    ...
    void removeOrder(String id) {
        orders.stream()
                .filter(item -> item.getId().equals(id))
                .findFirst()
                .ifPresent(item -> item.status = Status.DELETED);
    }
    ...
    void closeOrder(String id) {
        orders.stream()
                .filter(item -> item.getId().equals(id))
                .findFirst()
                .ifPresent(item -> item.status = Status.CLOSED);
    }

    public void rescheduleOrder(String id, TimeInterval newTimeInterval) {
        Order copy = orders.stream()
                .filter(item -> item.getId().equals(id))
                .findFirst()
                .orElse(null);
        if (copy == null) {
        ...
        Order newOrder = new Order(copy.name, copy.price, copy.masterIds, copy.workplaceId, newTimeInterval);
        removeOrder(id);
    ...
    public void printOrders() {
        for (var order : orders) {
            System.out.println(
                    order.name + " " + workplaces.show().stream()
                            .filter(o -> o.getId().equals(order.workplaceId))
                            .map(o -> o.name)
                            .findFirst()
                            .orElse("Unknown") +        // если не найдено
                    " " + order.timeInterval.start() + " " + order.timeInterval.end() + " masters:"
            );
            for (var mid : order.masterIds) {
                System.out.print("    " + masters.show().stream()
                        .filter(o -> o.getId().contains(mid))
                        .map(o -> o.name)
                        .findFirst()
                        .orElse("Unknown")
                );
    ...
    public List<Workplace> freeWorkplacesList() {
        ...
        for (Workplace workplace : workplaces.show()) {
            ...
            List<Order> openedOrders = orders.stream()
                    .filter(order -> order.status == Status.OPENED)
                    .filter(order -> order.workplaceId.equals(workplace.getId()))
                    .sorted(comparing(order -> order.timeInterval.start()))
                    .toList();

            List<TimeInterval> freeTimeIntervals = new ArrayList<>();

            for (TimeInterval interval : workplace.getWorkingSchedule()) {
                LocalDateTime freeIntervalStart = interval.start();

                for (Order order : openedOrders) {
                    if (order.timeInterval.end().isBefore(interval.start())) continue;  // заказ закончился до интервала
                    if (order.timeInterval.start().isAfter(interval.end())) break;      // заказ начался после интервала

                    if (order.timeInterval.start().isAfter(freeIntervalStart)) {
                        freeTimeIntervals.add(new TimeInterval(freeIntervalStart, order.timeInterval.start()));
                    }

                    // Сдвигаем freeIntervalStart за конец заказа
                    if (order.timeInterval.end().isAfter(freeIntervalStart)) {
                        freeIntervalStart = order.timeInterval.end();
                    }
                }
    ...
    public List<Order> orderList(OrdersSortParam param, SortType type) {
        Comparator<Order> comparator = switch (param) {
            case DATE -> comparing(order -> order.timeInterval.start());
    ...
    public List<Order> currentOrdersList(CurrentOrdersSortParam param, SortType type) {
        Comparator<Order> comparator = switch (param) {
            case START_TIME -> comparing(order -> order.timeInterval.start());
            case END_TIME -> comparing(order -> order.timeInterval.end());
            case PRICE -> comparing(order -> order.price);
        };
        if (type == SortType.DESC) {
            comparator = comparator.reversed();
        ...
        return orders.stream()
                .filter(order -> order.status == Status.OPENED)
                .filter(order -> order.timeInterval.start().isBefore(LocalDateTime.now()) &&
                        order.timeInterval.end().isAfter(LocalDateTime.now())
                )
                .sorted(comparator)
                .toList();
    }
    ...
    public Order currentOrderOfMaster(Master master) {
        return orders.stream()
                .filter(order -> order.status == Status.OPENED)
                .filter(item -> item.timeInterval.start().isBefore(LocalDateTime.now()) &&
                        item.timeInterval.end().isAfter(LocalDateTime.now())
                )
    ...
    public List<Order> currentOrdersList(Status status, TimeInterval searchTimeInterval, CurrentOrdersSortParam param,
                                         SortType type) {
        Set<Status> validStatuses = Set.of(Status.CANCELLED, Status.CLOSED, Status.DELETED);
        List<Order> result = orders.stream()
                .filter(order -> order.timeInterval.start().isAfter(searchTimeInterval.start()) &&
                        order.timeInterval.end().isBefore(searchTimeInterval.end())
                )
                .filter(order -> validStatuses.contains(order.status))
                .toList();
        Comparator<Order> comparator = switch (param) {
            case START_TIME -> comparing(order -> order.timeInterval.start());
            case END_TIME -> comparing(order -> order.timeInterval.end());
...
//    public Params nextFreeTimeInterval() {
//        List<Workplace> result = new ArrayList<>();
//
//        for (Workplace workplace : workplaces.show()) {
//            // Все открытые заказы для этого рабочего места, отсортированные по времени начала (причем, пересекаться не могут)
